...
def noise_filter(audio_data, fs, cutoff=1000, order=6):
...
def get_audio(duration=3, fs=16000):
    st.write("Listening... Please wait for a moment.")
    time.sleep(1)  
    ...
    filtered_audio = noise_filter(audio_data.flatten(), fs)
    audio_file_path = "recording.wav"
    write(audio_file_path, fs, filtered_audio.astype(np.int16)) 
...
def transcribe_audio(audio_file_path):
    try:
        ...
        if response.status_code == 200:
            ...
            st.write("Query Generated by listening:", text)
            return text
        else:
...
def main():
    ...
    with st.sidebar:
        ...
        if api_key or st.session_state.openai_key_entered:
            ...
            if st.button("Tap to generate Query"):
